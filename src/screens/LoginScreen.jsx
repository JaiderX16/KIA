...
const LoginScreen = ({ navigate }) => (
  <div className="min-h-full flex flex-col md:flex-row relative">
    <div className="hidden md:flex flex-1 bg-gradient-to-br from-blue-600/90 to-indigo-600/90 p-12 flex-col justify-between text-white relative overflow-hidden">
    ...
    <div className="w-full md:w-[500px] p-8 md:p-12 pt-20 flex flex-col justify-center z-10 relative">
      ...
      <h2 className="text-3xl md:text-4xl font-extrabold text-gray-800 mb-2">Bienvenido</h2>
      <p className="text-gray-500 mb-10 text-base font-medium">Inicia sesión para continuar</p>
      ...
      <div className="space-y-4 mb-8">
        ...
        <div className="relative group">
          ...
          <input type="password" placeholder="Contraseña" className="w-full pl-12 pr-12 py-4 md:py-5 bg-white/50 backdrop-blur-sm border border-white/80 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:bg-white/80 transition-all shadow-sm text-gray-700" defaultValue="password123"/>
          <Eye className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer transition-colors" size={20} />
        </div>
        <div className="text-right">
          <a href="#" className="text-sm text-blue-600 font-semibold hover:text-blue-700">¿Olvidaste tu contraseña?</a>
        </div>
      </div>

      <div className="mb-8">
        <p className="text-sm font-semibold text-gray-600 mb-4 px-1">Selecciona tu rol</p>
        <div className="flex justify-between gap-3">
          {['Estudiante', 'Docente', 'Personal'].map((role, idx) => (
            <button key={role} className={`flex-1 flex flex-col items-center p-4 rounded-2xl backdrop-blur-md transition-all duration-300 shadow-sm
                ${idx === 0 ? 'bg-blue-500 text-white border border-blue-400 shadow-[0_4px_12px_rgba(59,130,246,0.3)]' : 'bg-white/50 text-gray-500 border border-white hover:bg-white/70'}`}>
      ...
      <div className="mt-auto mb-4">
        ...
        <p className="text-center text-sm text-gray-500 mt-6 font-medium">
          ¿No tienes cuenta? <a href="#" className="text-blue-600 font-bold hover:underline">Regístrate</a>
        </p>
      </div>
    </div>
  </div>
